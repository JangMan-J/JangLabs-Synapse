...
**Fixed at:** 2026-06-12T17:30:00Z
**Source review:** .planning/phases/02-routing-index-live-recall-cutover/02-REVIEW.md
**Iterations:** 3 (1: fixer agent, WR-01..WR-07 · 2: re-review found WR-08 · 3: orchestrator-inline fix of WR-08)

## Iteration 3: WR-08 (orchestrator-inline)

The iteration-2 re-review validated all 7 fixes and surfaced one new Warning: **WR-08** —
the cheap gate judged only the FIRST segment's leading word of a compound command, while
the post-WR-03 engine tokenizes every `;`/`&&`/`||`/`|`/`\n` segment, so e.g.
`ls -la; nvidia-smi` (and its multiline equivalent) was gated silent while the engine fires.

**Fix** (`fix(02): WR-08 segment-wise generic gate`): the gate now normalizes `&&`/`||`/`|`
to `;`, splits segments, and skips only when EVERY segment's leading word is generic —
mirroring the engine's separator set exactly. The stale WR-01 comment guarantee is now true.

**Process deviation:** applied inline by the orchestrator (single precise shell edit in a
file fully in context) instead of spawning a third fixer agent — token/memory conservation
during an autonomous run; verification rigor unchanged.

**Verification:** WR-08 repro `ls -la\nnvidia-smi` (JSON-escaped) now fires through the real
hook; `ls; pwd` still gates silent with zero Python spawn; malformed-JSON/store-write/single-
generic edge gates unchanged; probes 10/10 fixture + live; `pytest tests/` 287 passed /
10 skipped / 146 subtests; `bench_recall.sh -n 20` p95=50ms gate=PASS.
